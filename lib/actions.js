...
Actions.prototype._showKadiraLink = function() {
  var packagesFile = path.join(this.config.app, '.meteor/packages');
  var packages = fs.readFileSync(packagesFile, 'utf-8');
  var hasKadira = kadiraRegex.test(packages);
  if(!hasKadira) {
    console.log(
      '“ Checkout '+'Kadira'.bold+
      ', our performance monitoring tool for Meteor.\n'+
      '  To learn more, go to http://kadira.io/mup ”\n'
    );
  }
}
